...
def upload_image_to_api_with_retry(image_path, tray_number, count, avg_length, avg_weight, bounding_boxes, masks, max_retries=2):
    ...
    for attempt in range(max_retries + 1):
        print(f"ðŸ“¤ Upload attempt {attempt + 1}/{max_retries + 1} for Tray {tray_number}")
        ...
        if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp')):
            ...
            if prediction_results is not None:
                ...
                if not upload_success:
                    print(f"âš ï¸  Image upload failed for Tray {tray_number}, but metrics were sent via MQTT")
